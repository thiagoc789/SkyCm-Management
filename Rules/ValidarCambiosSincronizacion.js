export default function sincronizacionAuto(context) {
		...
		if (xhr.readyState == 4) {
			if (xhr.status >= 200 && xhr.status < 304) {
				...
				//	if (provider.hasPendingDownload() == false || provider.isRequestQueueEmpty() == false || provider.hasPendingUpload==false) {	
					//dialog.alert("hay cambios pendientes");
					return context.executeAction('/SkyCM_V2/Actions/Service/UploadOnly.action');
