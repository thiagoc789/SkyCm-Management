...
function system_date(dateString, formatTokens) {
  ...
  formatTokens = formatTokens ? formatTokens : "MM";
...
function system_date1(dateString, formatTokens) {
  var theDate = dateString ? new Date(dateString) : new Date();
  ...
  // Sets default format token.
  formatTokens = formatTokens ? formatTokens : "yyyy";
  for (var property in transformMap) {
  	formatTokens = replace(formatTokens, property, transformMap[property]);
  }
...
export default function ContarVisitasPorMes(clientAPI) {
 	return clientAPI.count('/SkyCM_V2/Services/SkyCM_V2.service', 'CLIENTEINFO', "$filter= PERNR_SCP eq '{{#Application/#ClientData/#Property:UserId}}' ").then((count) => {
		...
		// If “Customers” Entity set is availale, then it return the total customers
//	$filter=ID_CAMP eq 'PV' and PERNR_SCP eq '{{#Application/#ClientData/#Property:UserId}}' and ANO eq 2020 and MES eq 09
	return clientAPI.count('/SkyCM_V2/Services/SkyCM_V2.service', 'HISTV', "$filter=ID_CAMP eq 'PV' and PERNR_SCP eq '{{#Application/#ClientData/#Property:UserId}}' and MES eq '"+system_date()+"' and ANO eq '"+system_date1()+"' ").then((count1) => {
		clientAPI.getPageProxy().getClientData().EquipmentTotalCount = count1;
		// If “Customers” Entity set is availale, then it return the total customers
	var	result = (((count1/count)*100));
	var Result1 = Math.round(result);
  
	var ResultFinal = ((Result1)+" %");
